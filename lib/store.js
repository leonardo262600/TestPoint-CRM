import fs from 'fs/promises';
import path from 'path';

const dbPath = path.join(process.cwd(), 'data', 'db.json');

const readDb = async () => {
  const content = await fs.readFile(dbPath, 'utf8');
  return JSON.parse(content);
};

const writeDb = async (data) => {
  await fs.writeFile(dbPath, JSON.stringify(data, null, 2), 'utf8');
};

export const getUsers = async () => {
  const db = await readDb();
  return db.users || [];
};

export const getClients = async () => {
  const db = await readDb();
  return db.clients || [];
};

export const addUser = async (user) => {
  const db = await readDb();
  const nextId = String(Date.now());
  db.users = db.users || [];
  const payload = {
    id: nextId,
    ...user,
    email: String(user.email || '').trim().toLowerCase(),
  };
  db.users.push(payload);
  await writeDb(db);
  return payload;
};

export const updateUser = async (updatedUser) => {
  const db = await readDb();
  const payload = {
    ...updatedUser,
    email: String(updatedUser.email || '').trim().toLowerCase(),
  };
  db.users = db.users.map((user) => (user.id === payload.id ? payload : user));
  await writeDb(db);
  return payload;
};

export const getProfiles = async () => {
  const db = await readDb();
  return db.profiles || [];
};

export const upsertProfile = async (profile) => {
  const db = await readDb();
  db.profiles = db.profiles || [];
  const idx = db.profiles.findIndex((p) => p.userId === profile.userId);
  if (idx >= 0) {
    db.profiles[idx] = profile;
  } else {
    db.profiles.push(profile);
  }
  await writeDb(db);
  return profile;
};

export const getBanking = async () => {
  const db = await readDb();
  return db.banking || [];
};

export const upsertBanking = async (entry) => {
  const db = await readDb();
  db.banking = db.banking || [];
  const idx = db.banking.findIndex((b) => b.userId === entry.userId);
  if (idx >= 0) {
    db.banking[idx] = entry;
  } else {
    db.banking.push(entry);
  }
  await writeDb(db);
  return entry;
};

export const addClient = async (client) => {
  const db = await readDb();
  const nextId = String(Date.now());
  db.clients = db.clients || [];
  db.clients.push({ id: nextId, status: 'Nuevo', ...client });
  await writeDb(db);
  return { id: nextId, status: 'Nuevo', ...client };
};

export const getTickets = async () => {
  const db = await readDb();
  return db.tickets || [];
};

export const addTicket = async (ticket) => {
  const db = await readDb();
  const nextId = 'T-' + String(Date.now());
  db.tickets = db.tickets || [];
  const newTicket = {
    id: nextId,
    status: 'nuevo',
    reply: null,
    repliedAt: null,
    creatorSeenAt: null,
    createdAt: new Date().toISOString(),
    ...ticket,
  };
  db.tickets.push(newTicket);
  await writeDb(db);
  return newTicket;
};

export const updateTicket = async (id, changes) => {
  const db = await readDb();
  db.tickets = db.tickets || [];
  const idx = db.tickets.findIndex((t) => t.id === id);
  if (idx < 0) return null;
  const nextChanges = { ...changes };
  if (nextChanges.reply !== undefined) {
    nextChanges.creatorSeenAt = null;
  }
  db.tickets[idx] = { ...db.tickets[idx], ...nextChanges };
  await writeDb(db);
  return db.tickets[idx];
};

...
const getSecret = () => {
  const secret = process.env.CRM_AUTH_SECRET || 'dev-insecure-change-me';
  if (process.env.NODE_ENV === 'production' && secret === 'dev-insecure-change-me') {
    throw new Error('CRM_AUTH_SECRET no está configurado. Define esta variable de entorno antes de iniciar en producción.');
  }
  return secret;
};
